...
def generate_examples_from_file(path: str, number_of_examples=10):
    ...
    example_smell: list = generate_examples_new(smell_tracker, number_of_examples)
...
def generate_examples(smell_tracker: list, number_of_examples=10):
    ...
    for s in smell_tracker:
        if s["type_components_affected"] == "UNIT" and len(s["characteristics_by_version"]) > 1 and (
                ...
                s["characteristics_by_version"][1]["ATDI_var"]["variation"] == "DOWN"):
            examples.append(s)
...
def generate_examples_new(smell_tracker: list, number_of_examples=10):
    examples: list = []

    for s in smell_tracker:
        if s["type_components_affected"] == "UNIT" and "new" in s:
            examples.append(s)
            number_of_examples -= 1
            if number_of_examples == 0:
                break
